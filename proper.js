...
class SnapLensProper {
  constructor() {
    ...
    this.statusEl = document.getElementById('status'); // optional
    this.cameraKit = null;
    this.session = null;
  ...
  async initializeApp() {
    ...
    // Do NOT auto-start; we start on first gesture via ensureStartedOnce()
  }
  ...
  async initializeCameraKit() {
    try {
      ...
      // Replace placeholder with the live canvas
      ...
      const resizeLiveCanvas = () => {
        ...
        console.log(
          `Canvas sized: ${this.liveCanvas.width}x${this.liveCanvas.height} (CSS: ${rect.width}x${rect.height}, DPR: ${dpr})`
        );
      };
  ...
  async ensureStartedOnce() {
    ...
    try {
      ...
      // iOS motion/orientation permission (needed for environment/world stability)
  ...
  async startCamera() {
    ...
    // Prefer deviceId when available (more reliable than facingMode)
    ...
    const constraints = {
      video: deviceId
        ? { deviceId: { exact: deviceId }, frameRate: { ideal: 30, max: 30 } }
    ...
    const source = createMediaStreamSource(this.mediaStream, {
      cameraType: wantFront ? 'user' : 'environment' // critical for world AR
    });
  ...
  async requestMotionPermissions() {
    try {
      ...
      if (!isIOS || !hasAPIs) return;

      // Must be called within a user gesture; our start/switch are gesture-driven
      if (
      ...
      console.warn('Motion permission request failed or not needed:', e);
    }
  }
  ...
  async switchCamera() {
    ...
    try {
      ...
      // Stop old tracks, attach fresh source
      if (this.mediaStream) {
        this.mediaStream.getTracks().forEach((t) => t.stop());
        this.mediaStream = null;
      }
      ...
      // Make sure motion permission is granted when going to environment
  ...
  setupBackgroundAudio() {
    ...
    // Try autoplay; if blocked, gesture listeners will handle it
